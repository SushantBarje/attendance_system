...
<?php   include "studentHeader.php" ?>
            ...
            <div class="col-12">
                <div class="table-section">
                    <table id="student-report-practical">
                        ...
                        <tbody>
                            <?php 
                                $result = $user->showGrandReportPractical();
                                foreach($result['data'] as $r){
                                    echo '<tr>
                                            <td class="details-control" id="'.$r['p_class_id'].'"></td>
                                            <th>'.$r['course_name'].'</th>
                                            <td>'.$r['present'].'</td>
                                            <td>'.$r['absent'].'</td>
                                            <td>'.$r['total_lectures'].'</td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#student-report-practical tbody').on('click', 'td.details-control', function () {
                var tr = $(this).closest('tr');
                ...
                if ( tr.hasClass("shown") ) {
                    ...
                    $.ajax({
                        url: "../controller/studentajaxController.php?action=get_student_class_pract_report",
                        type: "post",
                        data : {"data" : data},
                        dataType : "json",
                        success :  function(res){
