...
class Faculty extends Database
{
	...
	*/
	...
	public function getStudentByYear($data)
	{
		try {
			$con = $this->connect();
			$sql = "SELECT * FROM student WHERE year_id = ? AND dept_id = ?";
			$stmt = $con->prepare($sql);
	...
	*/
	...
	public function getDepartmentByName($name)
	{
		try {
			...
			$stmt = $con->prepare($sql);
			if ($stmt->execute($name)) return $stmt->fetchAll();
		} catch (PDOException $e) {
			return array("e" => $e->getMessage());
		}
	}
	...
	*/
	...
	public function getHodReportYearWise($query, $data)
	{
		try {
			...
			$sql = "SELECT GROUP_CONCAT(DISTINCT CONCAT( 'SUM(IF(a.class_id = ''', c.class_id, ''', a.status, NULL)) AS ', CONCAT('`',courses.course_name,'`'))) as string FROM courses RIGHT JOIN class as c ON courses.course_id = c.course_id LEFT JOIN attendance as a ON a.class_id = c.class_id WHERE " . $query . ";";
			$stmt = $con->prepare($sql);
			if ($stmt->execute($data)) {
				$string = $stmt->fetch()['string'];
				$sql = "
						SELECT  b.roll_no, CONCAT(b.last_name,'' '', b.first_name,'' '', b.middle_name) as student_name, a.student_id, SUM(a.status) as total, (SUM(a.status)/COUNT(a.status))*100 as percent,
						" . $string . "
						FROM attendance as a JOIN student as b 
						ON a.student_id = b.prn_no JOIN class as c 
						ON a.class_id = c.class_id 
						WHERE " . $query . "
						GROUP BY a.student_id ORDER BY b.roll_no+0;";
				...
				$sql = "
						SELECT COUNT(DISTINCT date_time) as lectures FROM courses 
						RIGHT JOIN class as c ON courses.course_id = c.course_id LEFT JOIN attendance as a ON a.class_id = c.class_id 
						WHERE " . $query . " GROUP BY c.class_id ORDER BY c.class_id;";
				$stmt = $con->prepare($sql);
				$stmt->execute($data);
				$lecture = $stmt->fetchAll();
				return array("report" => $report, "lectures" => $lecture);
			}
