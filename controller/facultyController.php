...
class FacultyController extends Faculty
{
    ...
    public function userLogin()
    {
        ...
        if ($result && $result[0]['faculty_id'] == $this->faculty_id && password_verify($this->password, $result[0]['password'])) {
            ...
            if (isset($result[0]['dept_id'])) {
                $_SESSION['dept'] = $result[0]['dept_id'];
                $_SESSION['dept_name'] = $result[0]['dept_name'];
            }
    ...
    public function savePracticalAttendance()
    {
        ...
        if (count($result) == 0) {
            ...
            foreach ((array)$this->attend as $key => $value) {
                ...
                $result = $this->insertStudentPracticalAttendance([(int)$this->class, $key, (int)$value, $timestamp, (int)$course_id['course_id']]);
                //var_dump($result);
                $c++;
            }
        } else {
